...
} from './LandscapeTimerDisplayContext';
...
const formatTime = ms => {
  ...
  return `${safeMinutes.toString().padStart(2, '0')}:${safeSeconds
    .toString()
    .padStart(2, '0')}.${safeMilliseconds.toString().padStart(3, '0')}`;
};

const Timer = React.memo(({onStart, onStop}) => {
  const {isRunning, isReadyToStart, isShowingResult, setTime, time, stopTime} =
    useLandscapeTimerDisplay();
...
const InnerLandscapeTimerDisplay = ({
  onTimerStart,
  onTimerStop,
  onStopTimerReady,
  onBestTimeUpdate,
  ...
  useEffect(() => {
    if (!isRunning && time !== '00:00.000') {
      ...
      Promise.all([saveTime(time)]).then(() => {
        loadData();
      });
    }
  }, [isRunning, time]);
  ...
  const loadData = async () => {
    try {
      ...
      // Cập nhật best time
      if (times.length > 0) {
        const bestTimeValue = Math.min(
          ...times.map(t => convertTimeToMs(t.time)),
        );
  ...
  const onLongPress = (side, event) => {
    if (event.nativeEvent.state === State.ACTIVE) {
      ...
      if (
        (side === 'left' && rightPressed) ||
        (side === 'right' && leftPressed)
      ) {
        if (!isRunning) {
    ...
    return (
      <>
        <TouchableOpacity
          ...
          <Text
            style={[styles.timeItemText, isAverage && styles.averageTimeText]}>
            {time}
          </Text>
        </TouchableOpacity>
          ...
          <View style={styles.statsBoxContainer}>
            <View style={styles.infoBox}>
              <Text style={styles.labelText}>
                Count sub {settings.numSubCount}s
              </Text>
              <View style={styles.averageTimeItem}>
